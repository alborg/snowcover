 ...
 */
int nc_get_mode(const int ncid,int *mode) {
   int status = nc_redef(ncid);   /* try to change to DEFINE mode */
 ...
 *
 */
int nc_get_var_strings(rsprod_data *data, rsprod_dimensions *dims, int ncid, int varid) {
   ...
   if (!isValidStringLengthDimensionName(lastDimName)) {
      fprintf(stderr,"ERROR (nc_get_var_strings) Invalid name for the 'number of character' dimension in nc_char dataset: <%s>\n",
	    lastDimName);
      return 1;
   }
   ...
   for ( size_t e = 0 ; e < dims->totelems ; e+=maximum_string_length ) {
      ...
      /*
      printf("VERBOSE (nc_get_var_strings) e:%u. index_of_string:%u. start is [",e,index_of_string);
      ...
      */
      ...
      ret = nc_handle_status(nc_get_vara_text(ncid,varid,start,count,stringValue));
      if (ret) {
	 fprintf(stderr,"ERROR (nc_get_var_strings) Could not write string at position [");
         for (size_t d = 0 ; d < ndims ; d++) {printf("%u ",start[d]);};
	 fprintf(stderr,"\b] in the netcdf dataset.\n");
	 return 1;
      }
      printf("VERBOSE (nc_get_var_strings): e:%u stringValue is\n\t<%s>\n",e,stringValue);
   }
 ...
 *
 */
int nc_get_var(rsprod_data *data, rsprod_dimensions *dims,int ncid, int varid) {
   ...
   switch(ncType) {
	 ...
	 break;
      case NC_CHAR:
	 ret = nc_get_var_strings(data,dims,ncid,varid);
	 // ret = nc_handle_status(nc_get_var_text(ncid,varid,values));
	 break;
      default:
	 ret = 1;
...
int nc_put_var(rsprod_field *this,int ncid, int varid) {
   ...
   if (nc_handle_status(nc_get_mode(ncid,&mode))) {
      fprintf(stderr,"WARNING (nc_put_var) Could not access the mode for nectdf id %d\n",ncid);
   } else {
      if (mode != NC_DAT_MODE) {
	 fprintf(stderr,"WARNING (nc_put_var) Call while ncid %d is in DEFINE mode.\n",ncid);
      }
   }
...
int nc_put_attr(rsprod_attr *attr,int ncid, int varid) {
   ...
   if (nc_handle_status(nc_get_mode(ncid,&mode))) {
      fprintf(stderr,"WARNING (nc_put_attr) Could not access the mode for netcdf id %d.\n",ncid);
   } else {
      if (mode != NC_DEF_MODE) {
	 fprintf(stderr,"WARNING (nc_put_attr) Call while ncid %d is in DATA mode.\n",ncid);
      }
   }
...
int rsprod_data_loadFromNetCDF(rsprod_data **this,rsprod_dimensions *dims,int ncid,int fieldid) {
   ...
   if (elems) {
      ret = nc_get_var(*this,dims,ncid,fieldid);
      if (ret) {
   ...
   if (rsprod_data_getType(this->data) == RSPROD_CHAR) {
	 ...
	 if (ret != NC_NOERR) {
	    if (ret != NC_EBADDIM) {
	       ...
	       fprintf(stderr,"ERROR (rsprod_field_preprocess_for_netcdf) problem in loading dimensions from netCDF file.\n");
	       return 1;
	    } else {
...
int rsprod_attributes_writeToNetCDF(rsprod_attributes this,const int ncid, const int fieldid) {
   ...
   if (ret) {
      fprintf(stderr,"ERROR (rsprod_attributes_writeToNetCDF) could not change to define mode.\n");
      return 1;
   }
