...
int rsprod_field_unpack(rsprod_field *this) {
   ...
   void *scaleFac = NULL;
   if (getAttributes(list,&attrScaleFactor,"scale_factor")) {
   ...
   void *offset = NULL;
   if (getAttributes(list,&attrOffset,"offset")) {
   ...
   void *fillval1 = NULL;
   if (getAttributes(list,&attrFillVal1,"_FillValue")) {
   ...
   if (rsprod_data_castToType(data,RSPROD_FLOAT,fillval1,fillval2,scaleFac,offset)) {
   ...
   if (replaceAttr(list,newFillValue)) {
...
rsprod_field *rsprod_field_copy(rsprod_field *orig) {
   ...
   ret += copyAttributes(orig->attr,&attr);
   if (ret) {
...
int rsprod_field_createStandard(rsprod_field **this, char *name, rsprod_type type, size_t nbvalues,
   ...
   if (longName) {
	 ...
	 ret++;
      ...
      ret += addcopyAttr(list,attr); rsprod_attr_delete(attr);
   }
   /** standard_name [opt] **/
   if (stdName) {
   ...
   if (addCoords) {
      char *listcoords;
      rsprod_dims_getListNames(cdims,&listcoords,' ');
