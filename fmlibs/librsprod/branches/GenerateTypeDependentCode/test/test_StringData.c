...
int main(int argc, char *argv[]) {
   ...
   ret = rsprod_dims_create(&outdims,1,dimnames,dimlengths,dimunlims);
   if (ret) {
   ...
   size_t nbstrings  = nx;
   size_t maxnbchars = chars2_in->dims->length[chars2_in->dims->nbdims-1];
   /* reformat into a array of strings */
   char **Sarray;
   ret = datastring_format(chars2_in_data,nx,maxnbchars,&Sarray);
   printf("Once reformatted:\n");
