...
int check_avhrr(char *ipath, char *basename, fstruct *avhrr_f,
    ...
    /*
     * Initialize the imgh structure to avoid memory leakage in fm_readheader
    ...
    */
    ...
    if (!basenamecopy) {
	fmerrmsg(where,"Could not create basenamecopy");
	return(3);
    }
    ...
    (*avhrr_f).n = nf;
    if (nf > FMCOLMAXFILES) {
	fmerrmsg(where,"Increase FMCOLMAXFILES");
	return(3);
    }
    if (nf == 0) return(0);
    if (((*avhrr_f).f) == NULL) {
	...
	if (fm_readheader(infile, &header) == 0) {
