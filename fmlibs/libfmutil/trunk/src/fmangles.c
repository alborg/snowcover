...
fmbool fmangest(fmangreq rs, fmangles *ang) {
    ...
    et = 0.170*sin(4*fmPI*(daynr-80.)/373.)
	-0.129*sin(2*fmPI*(daynr-8.)/355.);
     ...
     * the observation.
     */
    sinsunaz = (sin((fmPI/2)-incl)*sin(fabs(hangle)))/sin(ang->soz*DEG_TO_RAD);
	...
	fmerrmsg(where,"Failure at indicies: %d %d\n",rs.ind.col,rs.ind.row);
	return(FMFALSE);
    }
    /* 
    ...
    if (!fmsubtrn(lls, &stpos)) {
        fmerrmsg(where,"Could not locate nearest subtrack point");
	return(FMFALSE);
    }
