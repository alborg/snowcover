 ...
 * libsatimg.
 *
 * This routine is supposed to function on Polar Stereographic data using
 * the METNO specifiction with R=6371km, True Scale at 60N, and not
 * rotated compared to Greenwich.
 *
 * REQUIREMENTS:
 * NA
 *
 ...
 *
 * MODIFIED:
 * Øystein Godøy, METNO/FOU, 2013-12-01: Modified check of input
 * positions.
 *
 * ID:
 * $Id$
...
fmbool fmsubtrc(fmangreq rs, fmgeopos *p1, fmgeopos *p2) {
     ...
     * point and return positions with minimum distance to target.
    ...
    for (i=0; i<rs.subtrack.npoints; i++) {
        ...
        tll.lon = rs.subtrack.gpos[i].lon;
    ...
    /*
     * Originally this check was made against 170E, but do not remember
     * why. It is now changed to 180E. The original reason was probably
     * due to a smaller area of interest originally. The check for Equator
     * could also potentially be modified, but need careful checking as
     * this is a polar stereographic map projection.
     */
    if (p1->lat < 0 || p2->lat < 0 || p1->lon > 180 || p2->lon > 180){ 
