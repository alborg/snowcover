dnl <Note for met.no users.>
dnl This macros is intended to be used with autoheader. If you do not
dnl use autoheader and automatically generated config.h.in, this
dnl macro is useless for you.
dnl
dnl It was downloaded from the autoconf macro archive at:
dnl http://autoconf-archive.cryp.to/ax_prefix_config_h.html
dnl </Note for met.no users.>
dnl
dnl
...
dnl Example:
dnl
dnl   AC_INIT([Library foo],[0.1.1],[bugs@libfoo.net],[libfoo]) 
dnl   AM_INIT_AUTOMAKE([foreign])    
dnl   AM_CONFIG_HEADER(src/config.h)          # Instruct autoheader to create and use src/config.h.in
dnl   AX_PREFIX_CONFIG_H([src/foo_config.h],[FOO],[src/config.h]]) # prepend FOO to all macros in src/config.h
dnl                                                                # and put them in src/foo_config.h
dnl   AC_OUTPUT(Makefile)                 # creates the "src/config.h" now
dnl                                       # and also src/foo_config.h
dnl
